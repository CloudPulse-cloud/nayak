...
# AUTO-UPDATE CONFIG
# ══════════════════════════════════════════════════════════════
VERSION     = "1.0.6"   # bump this number when you push an update
...
def check_for_updates():
    ...
    # ── Success ────────────────────────────────────────────────
    # Play tone inline (play_tone not defined yet at module level)
    try:
        import ctypes, base64 as _b64, tempfile as _tmp
        if SUCCESS_TONE_B64:
            _bytes = _b64.b64decode(SUCCESS_TONE_B64)
            _ext   = '.wav' if _bytes[:4] == b'RIFF' else '.mp3'
            _fd, _tp = _tmp.mkstemp(suffix=_ext)
            import os as _os; _os.close(_fd)
            with open(_tp, 'wb') as _f: _f.write(_bytes)
            _winmm = ctypes.WinDLL('winmm')
            _buf   = ctypes.create_unicode_buffer(512)
            _winmm.mciSendStringW(f'open "{_tp}" alias updsnd', _buf, 512, 0)
            _winmm.mciSendStringW('play updsnd wait', _buf, 512, 0)
            _winmm.mciSendStringW('close updsnd', _buf, 512, 0)
            try: _os.unlink(_tp)
            except: pass
    except Exception:
        pass
