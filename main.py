...
# AUTO-UPDATE CONFIG
# ══════════════════════════════════════════════════════════════
VERSION     = "1.0.0"   # bump this number when you push an update
...
SUCCESS_TONE_B64 = ""   # baked in by embed_tones.py before uploading to GitHub
WARNING_TONE_B64 = ""   # baked in by embed_tones.py before uploading to GitHub
...
def mark_emails_sent(csv_file, email_list):
    with csv_lock:
        ...
        email_set = set(e.lower() for e in email_list)
        for row in rows:
            email_col = find_email_column(row)
            if not email_col:
                continue
...
def run_creation(client, account_id, tasks, profile, emails_csv_path, rate_limiter):
    ...
    def create_one(task):
        ...
        try:
            ...
            mark_emails_sent(emails_csv_path, task["sent_emails"])
            with lock:
                count[0] += 1
                print(f"  [OK]     '{task['name']}' — {len(task['sent_emails'])} emails | Done: {count[0]}")
...
def check_account_preflight(folder):
    """
    Checks one account folder without creating anything.
...
def process_account(folder, notification_type, profile, mode):
    print(f"\n{'='*60}")
    print(f"  Account: {os.path.basename(folder)}")
    print(f"{'='*60}")
    ...
    account_id = get_account_id(session)
    spending   = detect_spending(session)
    print(f"  Account ID : {account_id}")
    print(f"  Region     : {creds['region']}")
    print(f"  Spending   : ${spending}" if spending is not None else "  Spending   : unavailable")
    ...
    new_budgets  = preflight_checks(budgets, all_emails, existing)
    tasks        = build_budget_tasks(new_budgets, all_emails, notification_type)
    rate_limiter = RateLimiter(profile["RATE_LIMIT_PER_SEC"])

    print(f"  Starting budget creation + email assignment...")
    divider()
    ...
    for round_num in range(MAX_RETRY_ROUNDS + 1):
        ...
        print(f"  {'─'*40}")
        print(f"  Created  : {GREEN}{len(created)}{RESET}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        box("RUN SUMMARY", [
            ...
            f"Accounts Ready to Run  : {len(good_folders)}",
            ...
            f"Estimated Time (20k)   : ~{est_mins * max(1, len(good_folders) // accounts_in_parallel)} min total",
        ...
        # ── Phase 1: Check ALL accounts first ─────────────────
        print()
        divider()
