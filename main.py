...
def _handle_ctrl_c(sig, frame):
    global _SHUTDOWN
    _SHUTDOWN = True
    print(f"\n\n  {YELLOW if 'YELLOW' in dir() else ''}Ctrl+C detected — stopping after current batch...{RESET if 'RESET' in dir() else ''}")
    print(f"  Please wait a moment for threads to finish cleanly.")
...
# AUTO-UPDATE CONFIG
# ══════════════════════════════════════════════════════════════
VERSION     = "1.0.1"   # bump this number when you push an update
...
def check_account_preflight(folder):
    ...
    # Suppress all print output during silent check
    ...
    try:
        ...
        sys.stdout = _old_stdout
        return {"status":"error","name":name,"folder":folder,
                "account_id":"unknown","issues":[str(e)[:100]],
                "budgets":0,"emails":0}
